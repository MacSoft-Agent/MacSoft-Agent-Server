...
const DEFAULT_PORT_ANNOUNCE_TIMEOUT_MS = 120_000
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`MacSoft Agent service exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for MacSoft Agent service port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
