...
test('keeps the renderer surface narrow and listener cleanup explicit', async () => {
  ...
  assert.match(preload, /ipcRenderer\.invoke\('hermes:macsoft-admin:upload-file'/)
...
test('keeps stream ownership, validation, and SSE event filtering in Electron main', async () => {
  ...
  assert.match(main, /ipcMain\.handle\('hermes:macsoft-admin:upload-file'/)
  assert.match(main, /MACSOFT_ADMIN_MAX_UPLOAD_DATA_URL_BYTES/)
