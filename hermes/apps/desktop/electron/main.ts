...
function resolveHermesHome() {
  if (IS_PACKAGED) {
    return resolvePackagedRuntimeHome(process.env.MACSOFT_DATA_ROOT, process.env.ProgramData)
  }
...
if (IS_WINDOWS) {
  app.setAppUserModelId('com.macsoft.agent')
}

// MacSoft Agent owns the customer-facing version independently of the pinned
// internal runtime revision.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveMacSoftProductMetadata().product_version,
  copyright: 'Copyright © 2026 Mac Soft'
})
...
let bootProgressState = {
  ...
  message: 'Waiting to start MacSoft Agent services',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — MacSoft Agent will start automatically when it completes…',
      12
    )
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating MacSoft Agent — this window will close and the updater will open. Don’t reopen MacSoft Agent yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the MacSoft Agent installation open ' +
        '(a second MacSoft Agent window or a terminal running the agent). Close it and retry.'
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating MacSoft Agent (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart MacSoft Agent to retry.',
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting MacSoft Agent…', percent: 100 })
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen MacSoft Agent to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the MacSoft Agent desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen MacSoft Agent to finish.'
    }
  }
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart MacSoft Agent to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart MacSoft Agent to load the new version.',
      percent: 100
    })
...
function resolveHermesBackend(backendArgs) {
  ...
  return {
    ...
    label: 'MacSoft Agent is not installed yet; setup is required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'macsoft-host-managed') {
    throw new Error(
      'Chat execution is provided through MacSoft Server. Use Server & AutoCount Settings to manage services.'
    )
  }
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'MacSoft Agent recovery was handed off to Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('MacSoft Agent installation was cancelled.') as any
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for MacSoft Agent on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch MacSoft Agent.'
    )
  }
  ...
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'MacSoft Agent services are ready',
    progress: 82,
    running: true,
    error: null
  })
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the MacSoft Agent service.'
              )
            )
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to MacSoft Agent',
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'The remote MacSoft Agent service uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'A remote MacSoft Agent service is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote MacSoft Agent service.'
      )
    }
...
async function startHermes() {
  if (IS_MACSOFT_CUSTOMER_RUNTIME) {
    ...
    connectionPromise = (async () => {
      updateBootProgress({
        phase: 'backend.resolve',
        message: 'Connecting to MacSoft Agent configuration service',
        progress: 40,
        running: true,
        error: null
      })

      const connection = getMacSoftHostClient().configBackendConnection()
      await waitForHermes(connection.baseUrl, connection.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'MacSoft Agent configuration service is ready',
        progress: 100,
        running: true,
        error: null
      })
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving MacSoft Agent services', 8)
    ...
    if (remote) {
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote MacSoft Agent service is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', 'Resolving MacSoft Agent runtime', 28)
    const backend = await ensureRuntime(resolveHermesBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const hermesCwd = resolveHermesCwd()
    ...
    await advanceBootProgress('backend.port', 'Waiting for MacSoft Agent services to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for MacSoft Agent services to become ready', 90)
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'MacSoft Agent services are ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
ipcMain.handle('hermes:connection', async (_event, profile) => {
  ...
  if (IS_MACSOFT_CUSTOMER_RUNTIME) {
    ...
    const { token: _token, ...publicConnection } = connection

    return { ...publicConnection, token: null, wsUrl: null }
  }
...
function getMacSoftProductPaths() {
  return resolveMacSoftProductPaths({
    configuredDataRoot: process.env.MACSOFT_DATA_ROOT,
    configuredProgramRoot: process.env.MACSOFT_AGENT_ROOT,
...
function ensureMacSoftProductData(force = false): Promise<MacSoftProductInitializationResult> {
  if (!IS_PACKAGED) {
    return Promise.resolve({ firstRun: false, initialized: true })
  }
  if (force) {
    macSoftProductInitialization = null
  }
  if (!macSoftProductInitialization) {
    macSoftProductInitialization = initializePackagedProductData(getMacSoftProductPaths()).catch(error => {
      macSoftProductInitialization = null
      throw error
    })
  }
  return macSoftProductInitialization
}

function getServerAutoCountConfigService() {
  if (serverAutoCountConfigService) {
    return serverAutoCountConfigService
  }

  serverAutoCountConfigService = new ServerAutoCountConfigService({
    ensureProductData: IS_PACKAGED ? async () => void (await ensureMacSoftProductData(true)) : undefined,
    productPaths: getMacSoftProductPaths()
  })

  return serverAutoCountConfigService
}

ipcMain.handle('hermes:server-autocount:load', () => getServerAutoCountConfigService().loadSettings())
ipcMain.handle('hermes:server-autocount:model:load', () => getServerAutoCountConfigService().loadModelSettings())
ipcMain.handle('hermes:server-autocount:refresh-networks', () =>
  getServerAutoCountConfigService().refreshNetworkAddresses()
)
ipcMain.handle('hermes:server-autocount:save', (_event, payload) =>
  getServerAutoCountConfigService().saveSettings(payload)
)
ipcMain.handle('hermes:server-autocount:model:save', (_event, payload) =>
  getServerAutoCountConfigService().saveModelSettings(payload)
)
ipcMain.handle('hermes:server-autocount:test-server', (_event, port) =>
  getServerAutoCountConfigService().testServer(port)
)
ipcMain.handle('hermes:server-autocount:get-pairing-code', () =>
  getMacSoftHostClient().pairingCode()
)
ipcMain.handle('hermes:server-autocount:test-ai-service', (_event, url) =>
  getServerAutoCountConfigService().testAiService(url)
)
ipcMain.handle('hermes:server-autocount:test-autocount', (_event, payload) =>
  getServerAutoCountConfigService().testAutoCount(payload)
)
function getMacSoftHostClient() {
...
ipcMain.handle('hermes:macsoft-host:set-autostart', (_event, enabled) =>
...
function resolveMacSoftProductMetadata() {
  try {
    return loadMacSoftProductMetadata(getMacSoftProductPaths(), process.resourcesPath)
  } catch {
    return {
      product: 'MacSoft Agent' as const,
      product_version: app.getVersion(),
      channel: 'stable',
...
function showAboutPanelFresh() {
  const metadata = resolveMacSoftProductMetadata()
  app.setAboutPanelOptions({
    applicationName: APP_NAME,
    applicationVersion: metadata.product_version,
    copyright: 'Copyright © 2026 Mac Soft'
  })
  app.showAboutPanel()
}
...
app.whenReady().then(async () => {
  ...
  if (IS_PACKAGED) {
    try {
      const initialized = await ensureMacSoftProductData()
      macSoftFirstRunNavigationPending = initialized.firstRun
    } catch (error) {
      console.error(error instanceof Error ? error.message : 'MacSoft Agent product initialization failed.')
    }
  }
  createWindow()
