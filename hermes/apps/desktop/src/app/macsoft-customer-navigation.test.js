...
test('customer settings redirect is limited to one pending first-run visit', () => {
    assert.equal(shouldOpenFirstRunSettings({ customerRuntime: true, firstRunPending: true, pathname: '/' }), true);
    assert.equal(shouldOpenFirstRunSettings({ customerRuntime: true, firstRunPending: false, pathname: '/' }), false);
});
test('development and non-root navigation never force customer settings', () => {
    assert.equal(shouldOpenFirstRunSettings({ customerRuntime: false, firstRunPending: true, pathname: '/' }), false);
    assert.equal(shouldOpenFirstRunSettings({ customerRuntime: true, firstRunPending: true, pathname: '/settings' }), false);
});
