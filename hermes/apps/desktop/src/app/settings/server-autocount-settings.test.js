...
describe('ServerAutoCountSettingsPage', () => {
    ...
    const hostStatus = vi.fn(async () => ({
        ...
        version: '0.1.0',
        services: {
            ai_service: { last_error: null, name: 'ai_service', owned: true, pid: 123, started_at: 1, status: 'running' },
            server: { last_error: null, name: 'server', owned: true, pid: 456, started_at: 1, status: 'running' }
        }
    }));
    const serviceAction = vi.fn(async (name, action) => ({
        last_error: null,
        name,
        owned: action !== 'stop',
    ...
    const setAutoStart = vi.fn(async (enabled) => enabled);
    beforeEach(() => {
    ...
    it('loads current values without exposing the existing API key', async () => {
        render(_jsx(ServerAutoCountSettingsPage, {}));
        ...
        expect(screen.getByPlaceholderText('Existing key configured · leave blank to keep').value).toBe('');
