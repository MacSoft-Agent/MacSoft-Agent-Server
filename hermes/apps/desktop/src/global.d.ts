...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      serverAutoCount: {
        getPairingCode: (port: number) => Promise<string>
        load: () => Promise<ServerAutoCountSettings>
        loadModel: () => Promise<MacSoftModelSettings>
        refreshNetworks: () => Promise<{ addresses: NetworkAddress[]; recommendedAddress: string | null }>
        save: (payload: SaveServerAutoCountInput) => Promise<SaveServerAutoCountResult>
        saveModel: (payload: MacSoftModelSettings) => Promise<SaveMacSoftModelSettingsResult>
        testServer: (port: number) => Promise<ReadableCheckResult>
        testAiService: (url: string) => Promise<ReadableCheckResult>
        testAutoCount: (payload: AutoCountTestInput) => Promise<ReadableCheckResult>
      }
      macSoftHost: {
        status: () => Promise<MacSoftHostStatus>
        serviceAction: (name: MacSoftServiceName, action: MacSoftServiceAction) => Promise<MacSoftServiceStatus>
