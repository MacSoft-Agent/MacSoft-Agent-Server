...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'MacSoft Agent の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'MacSoft Agent の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingHermesDesktop: 'MacSoft Agent を起動中…'
    },
    errors: {
      backgroundExited: 'MacSoft Agent バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に MacSoft Agent バックグラウンドプロセスが終了しました。',
    ...
    failure: {
      title: 'MacSoft Agent を起動できませんでした',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'MacSoft Agent バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateHermes: 'MacSoft Agent を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。MacSoft Agent を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'MacSoft Agent が応答を待っています。',
      turnDoneTitle: 'MacSoft Agent が完了しました',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を MacSoft Agent のデフォルトに戻しますか？',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は MacSoft Agent がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'MacSoft Agent が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'MacSoft Agent がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、MacSoft Agent をシステム設定に合わせます。',
      fontSizeTitle: '文字サイズ',
      fontSizeDesc: '読みやすさを向上させるため、文字サイズを調整します。この設定はアプリの再起動後も復元されます。',
      fontSizeOptions: { small: '小', normal: '標準', large: '大', extra_large: '特大', senior: 'シニア' },
      fontSizeApply: '適用',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、MacSoft Agent の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。MacSoft Agent を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    serverAutoCount: {
      ...
      restart: '再起動',
      refreshStatus: '状態を更新',
      autoStartWithWindows: 'Windows と同時にサービスを自動起動',
      administratorDetails: '管理者の詳細',
      administratorDetail: '管理者の詳細',
      hostVersion: 'ホストバージョン',
      aiServicePid: 'AI サービス PID',
      serverPid: 'サーバー PID',
      controlBoundary: '制御範囲',
      localHostOnly: 'ローカルホストインターフェースのみ',
      unavailable: '利用不可',
      notRunning: '未実行',
      ...
      autoStartUpdateFailed: '自動起動設定を更新できませんでした。',
      networkRefreshFailed: 'ネットワークインターフェースを更新できませんでした。',
      refreshFailed: '更新に失敗しました',
      clientUrlCopied: 'クライアント URL をコピーしました',
      copyManually: 'フィールドから URL を手動でコピーしてください。',
      copyFailed: 'コピーに失敗しました',
      pairingCodeFailed: 'ペアリングコードを取得できませんでした。',
      reviewFields: 'フィールドを確認して再試行してください。',
      ...
      retry: '再試行',
      serverPort: 'サーバーポート',
      serverPortDesc: 'クライアント向けポートです。MacSoft Client は AI サービスではなく、ここに接続します。',
      networkInterface: 'ネットワークインターフェース',
      networkInterfaceDesc: '物理 Wi-Fi または Ethernet を推奨します。仮想ネットワークと VPN アドレスも選択できます。',
      networkInterfaceAria: '選択したネットワークインターフェース',
      networkKinds: {
        ethernet: 'Ethernet',
        other: 'ネットワーク',
        vpn: 'VPN',
      ...
      recommended: '推奨',
      thisComputerOnly: 'このコンピューターのみ',
      clientUrl: 'クライアント URL',
      clientUrlDesc: '同じローカルネットワーク上の MacSoft Client デバイスとこの URL を共有します。',
      clientUrlLocalDesc: 'ローカル専用 URL です。他のコンピューターからは使用できません。',
      copyClientUrl: 'クライアント URL をコピー',
      pairingCode: 'ペアリングコード',
      getCode: 'コードを取得',
      serverNotRunning: 'MacSoft Server は実行されていません。',
      pairingCodeDesc: 'MacSoft Client のペアリング用ワンタイムコードを要求します。',
      refreshIp: 'IP を更新',
      testServer: 'サーバーをテスト',
      advancedAiService: '詳細 · AI サービス',
      serviceUrl: 'サービス URL',
      serviceUrlDesc: 'MacSoft Server が使用する内部サービスです。クライアント URL ではありません。',
      servicePort: 'サービスポート',
      servicePortDesc: '変更すると、ランタイム API Server ポートと MacSoft Server URL が同時に更新されます。',
      testAiService: 'AI サービスをテスト',
      autoCountConnection: 'AutoCount 接続',
      cloudUrl: 'Cloud URL',
      cloudUrlDesc: 'AutoCount Cloud API のベース URL。',
      apiKey: 'API キー',
      existingKeyPlaceholder: '既存のキーを設定済み · 空欄で保持',
      enterApiKeyPlaceholder: 'API キーを入力',
      hideApiKey: 'API キーを非表示',
      revealApiKey: '入力した API キーを表示',
      apiKeyDesc: '既存のキーはこのページに読み込まれません。保持するには空欄にし、Bearer 接頭辞は含めないでください。',
      connectorId: 'コネクター ID',
      connectorIdDesc: '登録済みのローカルコネクターを選択します。',
      companyId: '会社 ID',
      companyIdDesc: 'コネクターが公開する会社/帳簿を選択します。データベース情報は読み取り専用のテスト結果です。',
      testAutoCount: 'AutoCount 接続をテスト',
      saveDesc:
        '保存時にタイムスタンプ付きバックアップを作成し、検証済み設定ファイルを安全に置き換えます。再起動が必要なサービスは結果に表示されます。',
      saveAndApply: '保存して適用'
    },
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'MacSoft Agent がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'MacSoft Agent が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'MacSoft Agent が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを MacSoft Agent がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから MacSoft Agent 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'MacSoft Agent はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    config: {
      ...
      loading: 'MacSoft Agent の設定を読み込み中...',
    ...
    gateway: {
      ...
      intro:
        'MacSoft Agent はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している MacSoft Agent バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      localDesc:
        'ローカルホストでプライベートな MacSoft Agent バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの MacSoft Agent バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      ...
      restartingMessage: 'MacSoft Agent は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · MacSoft Agent ${version}` : ''} に接続しました`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。MacSoft Agent がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
  ...
  starmap: {
    ...
    emptyDesc: 'MacSoft Agent がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには MacSoft Agent を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには MacSoft Agent を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら MacSoft Agent が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'MacSoft Agent デスクトップを設定' },
    ...
    hermesActiveSessions: (version, count) => `MacSoft Agent ${version} · アクティブセッション ${count}`,
  ...
  profiles: {
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した MacSoft Agent 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。MacSoft Agent が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには MacSoft Agent バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'MacSoft Agent から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
  ...
  composer: {
    ...
    placeholderStarting: 'MacSoft Agent を起動中...',
    placeholderReconnecting: 'MacSoft Agent に再接続中…',
    placeholderFollowUp: 'フォローアップを送信',
    newSessionPlaceholders: [
      '何を作りますか？',
      'MacSoft Agent にタスクを与える',
      '何か考えていることはありますか？',
      '必要なことを説明してください',
      '何に取り組みますか？',
      '何でも聞いてください',
    ...
    attachUrlDesc: 'MacSoft Agent がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'MacSoft Agent にコミットと PR を任せる',
      agentShipPrompt:
  ...
  updates: {
    stages: {
      ...
      pull: 'もうすぐ完了…',
      pydeps: '仕上げ中…',
      update: 'MacSoft Agent を更新中…',
      rebuild: 'デスクトップアプリを再ビルド中…',
      restart: 'MacSoft Agent を再起動中…',
      done: '更新が完了しました',
      manual: 'ターミナルから更新',
    ...
    unsupportedMessage: 'このバージョンの MacSoft Agent はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの MacSoft Agent をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の MacSoft Agent バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'MacSoft Agent をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'MacSoft Agent は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために MacSoft Agent デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
      'MacSoft Agent アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に MacSoft Agent を再度開きます。更新中はご自分で MacSoft Agent を開き直さないでください。',
    applyingBodyBackend:
      'リモートバックエンドが更新を適用して再起動します。復帰すると MacSoft Agent が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 MacSoft Agent が自動的に再度開きます。',
  ...
  install: {
    ...
    oneTimeTitle: 'MacSoft Agent には一度限りのインストールが必要です',
    ...
    settingUpTitle: 'MacSoft Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の MacSoft Agent CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の MacSoft Agent インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。MacSoft Agent インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'MacSoft Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'MacSoft Agent はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'MacSoft Agent を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — MacSoft Agent を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に MacSoft Agent を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — MacSoft Agent が自動接続します',
    ...
    authorizeThere: 'そこで MacSoft Agent を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。MacSoft Agent をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'MacSoft Agent 推論ゲートウェイのステータス',
      agents: 'エージェント',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。MacSoft Agent は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'MacSoft Agent を再起動中...',
      askRestart: 'MacSoft Agent にサーバーの再起動を依頼',
      lookingRestart: taskId => `MacSoft Agent は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'MacSoft Agent はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      finishedRestarting: message =>
        `MacSoft Agent がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'MacSoft Agent がサーバーを再起動できませんでした。',
      stillWorking:
        'MacSoft Agent はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'MacSoft Agent が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'MacSoft Agent ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.hermes/config.yaml) に追加されます。MacSoft Agent はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'MacSoft Agent は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'MacSoft Agent は続行するための認証情報が必要です。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには MacSoft Agent を再起動してください。',
    restartToSaveImages: '画像を保存するには MacSoft Agent を再起動してください',
