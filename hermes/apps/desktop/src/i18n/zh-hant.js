...
export const zhHant = defineLocale({
    ...
    settings: {
        ...
        appearance: {
            ...
            pet: {
                title: '寵物',
                intro: '領養一隻懸浮在應用上的 petdex 動畫寵物，它會根據 MacSoft Agent 的狀態做出反應——工具執行時奔跑、成功時歡呼、出錯時沮喪。',
                restartHint: '寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 MacSoft Agent，然後回到此處。',
                scaleTitle: '大小',
        ...
        fieldDescriptions: defineFieldCopy({
            ...
            updates: {
                nonInteractiveLocalChanges: 'MacSoft Agent 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
            }
        }),
        about: {
        ...
        gateway: {
            ...
            title: '閘道連線',
            envOverride: '環境變數覆寫',
            intro: 'MacSoft Agent 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 MacSoft Agent 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
            appliesTo: '套用至',
            allProfiles: '全部設定檔',
            ...
            remoteTitle: '遠端閘道',
            remoteDesc: '將此桌面殼層連線至遠端 MacSoft Agent 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
    ...
    updates: {
        ...
        guiSkewBody: '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 MacSoft Agent 桌面應用程式（你的 AppImage / .deb / .rpm）以保持一致。',
        copy: '複製',
        copied: '已複製',
        done: '完成',
        applyingBody: 'MacSoft Agent 更新程式會在自己的視窗中接管，並在完成後自動重新開啟 MacSoft Agent。更新期間請勿自行重新開啟 MacSoft Agent。',
    ...
    install: {
        ...
        finishingTitle: '正在收尾',
        failedDesc: '某個安裝步驟失敗。在 Windows 上，如果另一個 MacSoft Agent CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 MacSoft Agent 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    ...
    onboarding: {
        ...
        autoBrowser: provider => `已在瀏覽器中開啟 ${provider}。請在那裡授權 MacSoft Agent，連線會自動完成，無需複製或貼上。`,
    ...
    assistant: {
        ...
        approval: {
            ...
            reject: '拒絕',
            alwaysTitle: '一律允許此指令？',
            alwaysDescription: pattern => `這會將「${pattern}」模式加入永久允許清單（~/.hermes/config.yaml）。MacSoft Agent 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
            alwaysAllow: '一律允許'
        },
        clarify: {
            notReady: '澄清請求尚未就緒',
