...
export const zh = {
    ...
    settings: {
        ...
        appearance: {
            ...
            pet: {
                title: '宠物',
                intro: '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 MacSoft Agent 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
                restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 MacSoft Agent，然后回到此处。',
                scaleTitle: '大小',
        ...
        fieldDescriptions: defineFieldCopy({
            ...
            updates: {
                nonInteractiveLocalChanges: 'MacSoft Agent 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
            }
        }),
        about: {
        ...
        gateway: {
            ...
            title: '网关连接',
            envOverride: '环境变量覆盖',
            intro: 'MacSoft Agent 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 MacSoft Agent 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
            appliesTo: '应用于',
            ...
            remoteTitle: '远程网关',
            remoteDesc: '将此桌面外壳连接到远程 MacSoft Agent 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
    ...
    messaging: {
        ...
        platformIntro: {
            ...
            bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 MacSoft Agent 指向该 URL。',
            ...
            weixin: '运行 `hermes gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。MacSoft Agent 会通过腾讯 iLink Bot API 连接并保存凭据。',
            qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
            api_server: '把 MacSoft Agent 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
    ...
    updates: {
        ...
        guiSkewBody: '后端已更新，但此桌面应用包未更改。请更新或重新安装 MacSoft Agent 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
        copy: '复制',
        copied: '已复制',
        done: '完成',
        applyingBody: 'MacSoft Agent 更新器会在自己的窗口中接管，并在完成后自动重新打开 MacSoft Agent。更新期间请不要自行重新打开 MacSoft Agent。',
    ...
    install: {
        ...
        finishingTitle: '正在收尾',
        failedDesc: '某个安装步骤失败。在 Windows 上，如果另一个 MacSoft Agent CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 MacSoft Agent 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    ...
    onboarding: {
        ...
        autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 MacSoft Agent，连接会自动完成，无需复制或粘贴。`,
    ...
    assistant: {
        ...
        approval: {
            ...
            reject: '拒绝',
            alwaysTitle: '始终允许此命令？',
            alwaysDescription: pattern => `这会将“${pattern}”模式加入永久允许列表 (~/.hermes/config.yaml)。MacSoft Agent 对类似命令将不再询问，包括当前会话和未来会话。`,
            alwaysAllow: '始终允许'
        },
        clarify: {
            notReady: '澄清请求尚未就绪',
