...
export const zh: Translations = {
  ...
  boot: {
    ready: 'MacSoft Agent 桌面版已就绪',
    desktopBootFailedWithMessage: message => `桌面启动失败：${message}`,
    steps: {
      connectingGateway: '正在连接桌面网关',
      loadingSettings: '正在加载 MacSoft Agent 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 MacSoft Agent 桌面版…'
    },
    errors: {
      backgroundExited: 'MacSoft Agent 后台进程已退出。',
      backgroundExitedDuringStartup: 'MacSoft Agent 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 MacSoft Agent 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    installMethodUnsupportedTitle: '不受支持的安装方式',
    updateHermes: '更新 MacSoft Agent',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 MacSoft Agent。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: 'MacSoft Agent 正在等待你的回应。',
      turnDoneTitle: 'MacSoft Agent 已完成',
      turnDoneBody: '回复已就绪。',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 MacSoft Agent 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 MacSoft Agent 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'MacSoft Agent 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'MacSoft Agent 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '本轮以错误结束。'
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 MacSoft Agent 跟随系统设置。',
      fontSizeTitle: '字体大小',
      fontSizeDesc: '调整文字大小以提升可读性。此设置会在应用重启后恢复。',
      fontSizeOptions: { small: '小', normal: '标准', large: '大', extra_large: '特大', senior: '长者模式' },
      fontSizeApply: '应用',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 MacSoft Agent 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint:
          '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 MacSoft Agent，然后回到此处。',
        scaleTitle: '大小',
    ...
    serverAutoCount: {
      title: '服务器与 AutoCount',
      intro: '配置面向客户端的 MacSoft Server、内部 AI 服务和 AutoCount Cloud 连接。保存不会自动启动或停止服务。',
      serviceControl: '服务控制',
      aiService: 'AI 服务',
      ...
      restart: '重启',
      refreshStatus: '刷新状态',
      autoStartWithWindows: '随 Windows 自动启动服务',
      administratorDetails: '管理员详情',
      administratorDetail: '管理员详情',
      hostVersion: '主机版本',
      aiServicePid: 'AI 服务 PID',
      serverPid: '服务器 PID',
      controlBoundary: '控制边界',
      localHostOnly: '仅限本地主机接口',
      unavailable: '不可用',
      notRunning: '未运行',
      noTestRun: '尚未运行连接测试。',
      next: '下一步',
      desktopBridgeUnavailable: '桌面配置桥接不可用。',
      settingsLoadFailed: '无法加载服务器与 AutoCount 设置。',
      hostBridgeUnavailable: '桌面主机桥接不可用。',
      hostUnavailable: 'MacSoft Agent 主机不可用。',
      serviceActionFailed: '服务操作失败。',
      autoStartUpdateFailed: '无法更新自动启动设置。',
      networkRefreshFailed: '无法刷新网络接口。',
      refreshFailed: '刷新失败',
      clientUrlCopied: '客户端 URL 已复制',
      copyManually: '请从字段中手动复制 URL。',
      copyFailed: '复制失败',
      pairingCodeFailed: '无法获取配对代码。',
      reviewFields: '请检查字段后重试。',
      connectionTestFailed: '无法完成连接测试。',
      autoCountTestFailed: 'AutoCount 测试失败',
      savedRestartRequired: services => `已安全保存。需要重启：${services}。`,
      savedNoRestart: '已安全保存。无需重启服务。',
      settingsSaved: '服务器与 AutoCount 设置已保存',
      settingsSaveFailed: '无法保存设置。',
      saveFailed: '保存失败',
      loading: '正在加载服务器与 AutoCount 设置…',
      loadPageFailed: '无法加载设置',
      retry: '重试',
      serverPort: '服务器端口',
      serverPortDesc: '面向客户端的端口。MacSoft Client 连接此端口，而不是 AI 服务。',
      networkInterface: '网络接口',
      networkInterfaceDesc: '建议使用实体 Wi-Fi 或以太网。仍可选择虚拟网络和 VPN 地址。',
      networkInterfaceAria: '所选网络接口',
      networkKinds: { ethernet: '以太网', other: '网络', vpn: 'VPN', virtual: '虚拟网络', wifi: 'Wi-Fi' },
      recommended: '推荐',
      thisComputerOnly: '仅此电脑',
      clientUrl: '客户端 URL',
      clientUrlDesc: '与同一本地网络上的 MacSoft Client 设备共享此 URL。',
      clientUrlLocalDesc: '仅限本机的 URL。其他电脑无法使用此地址。',
      copyClientUrl: '复制客户端 URL',
      pairingCode: '配对代码',
      getCode: '获取代码',
      serverNotRunning: 'MacSoft Server 未运行。',
      pairingCodeDesc: '请求用于配对 MacSoft Client 的一次性代码。',
      refreshIp: '刷新 IP',
      testServer: '测试服务器',
      advancedAiService: '高级 · AI 服务',
      serviceUrl: '服务 URL',
      serviceUrlDesc: 'MacSoft Server 使用的内部服务，不是客户端 URL。',
      servicePort: '服务端口',
      servicePortDesc: '更改此项会同时更新运行时 API Server 端口和 MacSoft Server URL。',
      testAiService: '测试 AI 服务',
      autoCountConnection: 'AutoCount 连接',
      cloudUrl: 'Cloud URL',
      cloudUrlDesc: 'AutoCount Cloud API 基础 URL。',
      apiKey: 'API 密钥',
      existingKeyPlaceholder: '已配置现有密钥 · 留空以保留',
      enterApiKeyPlaceholder: '输入 API 密钥',
      hideApiKey: '隐藏 API 密钥',
      revealApiKey: '显示输入的 API 密钥',
      apiKeyDesc: '现有密钥不会加载到此页面。留空可保留密钥；请勿包含 Bearer 前缀。',
      connectorId: '连接器 ID',
      connectorIdDesc: '选择已注册的本地连接器。',
      companyId: '公司 ID',
      companyIdDesc: '选择连接器公开的公司/账套。数据库详情只会作为只读测试结果显示。',
      testAutoCount: '测试 AutoCount 连接',
      saveDesc: '保存时会创建带时间戳的备份，并以原子方式替换已验证的配置文件。结果会列出需要重启的服务。',
      saveAndApply: '保存并应用'
    },
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 MacSoft Agent 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'MacSoft Agent 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'MacSoft Agent 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'MacSoft Agent 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'MacSoft Agent 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'MacSoft Agent 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'MacSoft Agent 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 MacSoft Agent 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 MacSoft Agent 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 MacSoft Agent 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'MacSoft Agent 将使用已保存设置重新连接。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · MacSoft Agent ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。MacSoft Agent 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 MacSoft Agent 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 MacSoft Agent 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 MacSoft Agent 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 MacSoft Agent 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 MacSoft Agent 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `MacSoft Agent ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 MacSoft Agent 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 MacSoft Agent 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 MacSoft Agent 指向该 URL。',
      ...
      weixin:
        '运行 `hermes gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。MacSoft Agent 会通过腾讯 iLink Bot API 连接并保存凭据。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 MacSoft Agent 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 MacSoft Agent 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。MacSoft Agent 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 MacSoft Agent 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 MacSoft Agent 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 MacSoft Agent…',
    placeholderReconnecting: '正在重新连接 MacSoft Agent…',
    placeholderFollowUp: '发送后续消息',
    newSessionPlaceholders: [
      '我们要构建什么？',
      '给 MacSoft Agent 一个任务',
      '你在想什么？',
      '描述你需要什么',
      '我们该处理什么？',
      '随便问点什么',
    ...
    attachUrlDesc: 'MacSoft Agent 将抓取该页面并作为本回合的上下文。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: '让 MacSoft Agent 提交并开 PR',
  ...
  updates: {
    stages: {
      ...
      pull: '马上完成…',
      pydeps: '收尾中…',
      update: '正在更新 MacSoft Agent…',
      rebuild: '正在重新构建桌面应用…',
      restart: '正在重启 MacSoft Agent…',
      done: '更新完成',
      manual: '从终端更新',
      guiSkew: '请更新桌面应用',
      error: '更新已暂停'
    ...
    unsupportedMessage: '此版本的 MacSoft Agent 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 MacSoft Agent 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 MacSoft Agent 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 MacSoft Agent，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 MacSoft Agent 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 MacSoft Agent 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      'MacSoft Agent 更新器会在自己的窗口中接管，并在完成后自动重新打开 MacSoft Agent。更新期间请不要自行重新打开 MacSoft Agent。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 MacSoft Agent 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 MacSoft Agent 会自动重新打开。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    oneTimeTitle: 'MacSoft Agent 需要一次性安装',
    ...
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 MacSoft Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 MacSoft Agent CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 MacSoft Agent 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。MacSoft Agent 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 MacSoft Agent',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'MacSoft Agent 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 MacSoft Agent…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 MacSoft Agent 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 MacSoft Agent 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — MacSoft Agent 会自动连接',
    ...
    authorizeThere: '请在那里授权 MacSoft Agent。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider =>
      `已在浏览器中打开 ${provider}。请在那里授权 MacSoft Agent，连接会自动完成，无需复制或粘贴。`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'MacSoft Agent 推理网关状态',
      agents: '代理',
      closeAgents: '关闭代理',
      openAgents: '打开代理',
      subagents: count => `${count} 个子代理`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。MacSoft Agent 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'MacSoft Agent 正在重启...',
      askRestart: '让 MacSoft Agent 重启服务器',
      lookingRestart: taskId => `MacSoft Agent 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'MacSoft Agent 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `MacSoft Agent 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'MacSoft Agent 无法重启服务器。',
      stillWorking: 'MacSoft Agent 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    thread: {
      ...
      showEarlier: '显示更早的消息',
      loadingResponse: 'MacSoft Agent 正在加载回复',
    ...
    approval: {
      gatewayDisconnected: 'MacSoft Agent 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.hermes/config.yaml)。MacSoft Agent 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'MacSoft Agent 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'MacSoft Agent 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 MacSoft Agent 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 MacSoft Agent 桌面版以保存图片',
