...
async function checkRuntime(ctx, requestedProvider) {
    if (window.hermesDesktop?.macSoftCustomerRuntime === true) {
        try {
            const options = await getGlobalModelOptions();
            const requested = requestedProvider?.trim().toLowerCase();
            const providers = options.providers ?? [];
            const candidates = requested
                ? providers.filter(provider => [provider.slug, provider.name].some(value => String(value || '').trim().toLowerCase() === requested))
                : providers;
            const ready = candidates.some(provider => provider.authenticated !== false || (provider.models?.length ?? 0) > 0);
            return {
                checksDisagree: false,
                ready,
                reason: ready ? null : DEFAULT_ONBOARDING_REASON,
                source: 'setup_status'
            };
        }
        catch (error) {
...
function notifyReady(provider) {
    notify({ kind: 'success', title: 'MacSoft Agent is ready', message: `${provider} connected.` });
}
...
function providerResolutionFailure(reason) {
    ...
    return detail
        ? `Connected, but MacSoft Agent still cannot resolve a usable provider. ${detail}`
        : 'Connected, but MacSoft Agent still cannot resolve a usable provider.';
}
...
export async function refreshOnboarding(ctx) {
    ...
    if (shouldPreserveConfiguredOnFallback(runtime, state)) {
        ...
        notify({
            ...
            message: 'MacSoft Agent could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
        });
        return false;
    }
...
export async function recheckExternalSignin(ctx) {
    ...
    await completeWithModelConfirm(ctx, provider.name, [provider.id], reason => setFlow({
        ...
        provider,
        message: reason?.trim() ||
            `MacSoft Agent still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    }));
}
...
export async function saveOnboardingLocalEndpoint(baseUrl, apiKey, ctx) {
    ...
    try {
        ...
        if (!runtime.ready) {
            ...
            return { ok: false, message: detail || `Saved, but MacSoft Agent still cannot reach ${url}.` };
        }
