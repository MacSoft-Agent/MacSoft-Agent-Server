    ...
    try {
      ...
      const candidates = requested
        ? providers.filter(provider =>
            [provider.slug, provider.name].some(value => String(value || '').trim().toLowerCase() === requested)
          )
        : providers
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'MacSoft Agent could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
