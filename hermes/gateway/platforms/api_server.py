...
def _resolve_macsoft_profile_home(request: "web.Request") -> tuple[Optional[Path], Optional["web.Response"]]:
    ...
    admin_scope = request.headers.get("X-MacSoft-Admin-Scope", "").strip()
    if profile_id and admin_scope:
        return None, web.json_response(_openai_error("Conflicting MacSoft scopes"), status=400)
    if admin_scope:
        shared_runtime = os.environ.get("HERMES_HOME", "").strip()
        if not shared_runtime:
            logger.error("MacSoft Admin scope rejected: HERMES_HOME is not configured")
            return None, web.json_response(
                _openai_error("MacSoft Admin routing is unavailable"), status=503
            )
        ...
        if not home.is_dir():
            return None, web.json_response(_openai_error("MacSoft Admin scope not found"), status=404)
        return home, None
    if not profile_id:
        return None, None
