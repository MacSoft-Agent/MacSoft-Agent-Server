...
async def _lifespan(app: "FastAPI"):
    ...
    # original eager import behaviour.
    if not _CONFIG_ONLY_MODE:
    ...
    # Config-only mode has no PTY surface, so it must not initialise that
    # subsystem or its reaper.
    pty_reaper_task = None if _CONFIG_ONLY_MODE else asyncio.create_task(run_reaper(PTY_REGISTRY))
...
# MacSoft's Host-managed configuration backend reuses the original Hermes
# config/model/provider/authentication routes without exposing a second Agent
# execution surface. This is read once at process start, before FastAPI serves
# requests, so the boundary cannot be toggled remotely.
_CONFIG_ONLY_MODE = os.environ.get("HERMES_CONFIG_ONLY") == "1"

# In-browser Chat tab (/chat, /api/pty, /api/ws, …). Normal Hermes dashboard
# launches keep it enabled. MacSoft's configuration-only process disables the
# whole surface before any handler can construct a tui_gateway Agent. Kept as a
# module-level seam so every WS endpoint and SPA token injection agree.
_DASHBOARD_EMBEDDED_CHAT_ENABLED = not _CONFIG_ONLY_MODE
...
@app.middleware("http")
async def config_only_route_boundary(request: Request, call_next):
    """Fail closed outside Hermes model/provider/configuration routes.

    In config-only mode this middleware is outermost (registered last), so
    developer, filesystem, terminal, PTY, session, tool and execution routes
    never reach their handlers. WebSocket endpoints have their own early-close
    seam through ``_DASHBOARD_EMBEDDED_CHAT_ENABLED``.
    """
    if _CONFIG_ONLY_MODE and not _config_only_http_path_allowed(request.url.path):
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        status = {
            "version": __version__,
            "runtime_mode": "config-only" if _CONFIG_ONLY_MODE else "dashboard",
            "release_date": __release_date__,
