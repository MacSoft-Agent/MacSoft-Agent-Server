...
description: Select meaningful KPIs and organize business dashboards.
---
...
## MacSoft data boundary

- Use only metrics and fields returned by a verified data source. Do not invent
  values, targets, benchmarks, or “real-time” status.
- Define every KPI's numerator, denominator, unit, time window, filters, and
  aggregation before placing it on a dashboard.
- Do not treat document counts as amounts, orders as completed sales, or missing
  values as zero without an explicit business rule.
- Read `references/details.md` only when the navigation summary is insufficient.
- The final dashboard is a single HTML document. Do not claim that an artifact
  was saved or previewed; those capabilities are not part of this phase.
