...
description: Build polished HTML dashboards and visual web artifacts.
---
...
## MacSoft delivery boundary

For MacSoft dashboard work, return one complete, self-contained HTML document
starting with `<!doctype html>` and ending with `</html>`. Use only verified data
already retrieved by the workflow; never add sample rows, guessed totals, or
unsupported business explanations. Use the MacSoft ECharts implementation
reference for chart options and mappings. This phase does not provide an
artifact store or Preview API, so do not claim that a file was saved or opened.
Do not assume a CDN, local library path, or network access. Use ECharts only
through an approved runtime delivery; otherwise provide a truthful HTML/CSS
fallback rather than inventing a dependency path.
...
For React prototypes outside the MacSoft dashboard workflow, use the pinned
runtime guidance in `references/advanced-patterns.md`. MacSoft dashboard output
must not assume CDN delivery or a React/Babel runtime.
...
- Use ECharts for approved MacSoft dashboard rendering, following the MacSoft
  chart reference; use semantic HTML/CSS when a chart runtime is unavailable.
- Responsive chart containers (`ResizeObserver`) are appropriate when the
  rendering runtime is present.
...
**Default to hand-written CSS and approved local/runtime resources.** Do not add
Chart.js, D3, AntV API calls, image URLs, or CDN scripts to a MacSoft dashboard.
If a future product contract explicitly supplies a resource, follow that
contract; otherwise keep the HTML dependency-free and state the limitation.
...
> React + Babel guidance in `references/advanced-patterns.md` applies only to
> non-MacSoft prototypes with an explicitly supplied runtime.
